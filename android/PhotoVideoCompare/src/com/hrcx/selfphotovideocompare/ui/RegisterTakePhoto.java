...
public class RegisterTakePhoto<_camera> extends Activity implements Callback {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mSurfaceView = (SurfaceView) findViewById(R.id.register_surface_camera);
		mSurfaceHolder = mSurfaceView.getHolder();
		mSurfaceHolder.addCallback(this);
		mSurfaceHolder.setType(SurfaceHolder.SURFACE_TYPE_PUSH_BUFFERS);

		progressDialog = new ProgressDialog(RegisterTakePhoto.this);
		// 构建进度条
	...
	public void upload() {
		try {
			// 上传图片
			Intent dataIntent = new Intent(RegisterTakePhoto.this,
					RegisterCommitPhoto.class);
			dataIntent.putExtra("curenImageUrl", IMAGEFILLPATH + currentImage);
			startActivityForResult(dataIntent, 1);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
		...
		ivFocus = new ImageView(RegisterTakePhoto.this);
