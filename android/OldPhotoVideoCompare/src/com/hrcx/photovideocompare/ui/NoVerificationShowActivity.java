...
import com.hrcx.photovideocompare.ui.TotalShowActivity.ActivityTask;
...
public class NoVerificationShowActivity extends BasicActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		view = mInflater.inflate(R.layout.activity_no_verification_show, null);
		setContentView(view);
		registerUpdate();
		findView();
	}
		...
		case R.id.btn_search:
			if (!isAccess) {
				...
				if (searchSfzh.length() != 15 && searchSfzh.length() != 18) {
					Toast.makeText(NoVerificationShowActivity.this,
							R.string.sfzh_below_15, Toast.LENGTH_SHORT).show();
				} else {
					isAccess = true;
					pd.setMessage(NoVerificationShowActivity.this.getParent()
		...
		case R.id.btn_lastpage:
			if (!isAccess) {
				...
				new LastPageActivityTask()
						.execute((Constant.noVerificationCurPage - 1) + "");
			}
			break;
		...
		btn_confirm.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (skipPage != null && skipPage.length() > 0) {
					...
					if (skipPageValue <= 0
							|| skipPageValue > Constant.noVerificationTotalPages) {
						Toast.makeText(NoVerificationShowActivity.this,
	...
	class ActivityTask extends AsyncTask<String, Boolean, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			return new NetProtocol(NoVerificationShowActivity.this)
					.noVerificationSearch(params[0]);
		}
		...
		protected void onPostExecute(Boolean result) {
			...
			if (!result) {
				if (Constant.isSessionExpire) {
					...
					Intent it = new Intent(NoVerificationShowActivity.this,
							LoginActivity.class);
					startActivity(it);
					finish();
					return;
	...
	class LastPageActivityTask extends AsyncTask<String, Boolean, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			return new NetProtocol(NoVerificationShowActivity.this)
					.syncNoVerificationData(params[0]);
		}
		...
		protected void onPostExecute(Boolean result) {
			...
			super.onPostExecute(result);
			pd.dismiss();
			Constant.noVerificationCurPage--;
			// 更新相关控件显示
	...
	class SkipPageActivityTask extends AsyncTask<String, Boolean, Boolean> {
		...
		protected void onPostExecute(Boolean result) {
			...
			super.onPostExecute(result);
			pd.dismiss();
			Constant.noVerificationCurPage = skipPageValue;
			// 更新相关控件显示
			updateRelativeControlShow();
		}
	}
	...
	class UpdateActivity extends AsyncTask<String, Boolean, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			return new NetProtocol(NoVerificationShowActivity.this)
					.syncNoVerificationData(Constant.totalCurPage + "");
		}
	...
	class UserInfoAdapter extends BaseAdapter {
		...
		@Override
		public int getCount() {
			return Constant.noVerificationUserInfoList.size();
		}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			final UserInfo userInfo = Constant.noVerificationUserInfoList
					.get(position);
			...
			switch (color) {
			case Constant.RED:
				tv_sfzh.setTextColor(NoVerificationShowActivity.this
