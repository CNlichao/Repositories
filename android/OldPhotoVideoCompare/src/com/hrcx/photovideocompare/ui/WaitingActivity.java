...
public class WaitingActivity extends BasicActivity {
	...
	class ValidationIsOkTask extends AsyncTask<String, Boolean, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			return new NetProtocol(WaitingActivity.this).isValidationOk();
		}
		...
		protected void onPostExecute(Boolean result) {			
			if (!result) {
				...
				if (Constant.isSessionExpire) {
					Toast.makeText(WaitingActivity.this, R.string.session_expire,
					...
					Toast.makeText(WaitingActivity.this, R.string.data_request_fail,
							Toast.LENGTH_SHORT).show();
					return;
				}
			}
			...
			if (Constant.waitTime <= 0) {
				if (Constant.isTimeOut) {
					Toast.makeText(WaitingActivity.this, R.string.check_time_out,
							Toast.LENGTH_SHORT).show();
					Constant.isTimeOut = false;
					finish();
				} else {
					Toast.makeText(WaitingActivity.this, R.string.data_request_success,
							Toast.LENGTH_SHORT).show();
					tv_waiting.setVisibility(View.GONE);
					tv_remain_seconds.setVisibility(View.GONE);
	...
	class GetPhotoesTask extends AsyncTask<String, Boolean, Boolean> {
		...
		protected Boolean doInBackground(String... params) {
			Boolean result = null;
			if (Constant.matchRatio >= 0.9) {
				result = new NetProtocol(WaitingActivity.this).getCheckPhotoes(Constant.pictureNum);
			} else {
				if (Constant.pictureNum < 3) {
					for (int i = 0; i <= Constant.pictureNum; i++) {
						result = new NetProtocol(WaitingActivity.this).getCheckPhotoes(i);
					}
				} else {
					for (int i = 1; i <= Constant.pictureNum; i++) {
						result = new NetProtocol(WaitingActivity.this).getCheckPhotoes(i);
					}
				}
			}
		...
		protected void onPostExecute(Boolean result) {
			pd.dismiss();
			if (!result) {
				Toast.makeText(WaitingActivity.this, R.string.photo_download_fail,
						Toast.LENGTH_SHORT).show();
				return;
			}
			super.onPostExecute(result);
			Toast.makeText(WaitingActivity.this, R.string.photo_download_success,
					Toast.LENGTH_SHORT).show();
			Intent intent = new Intent(WaitingActivity.this,
					CheckPhotoShowActivity.class);
